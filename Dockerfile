...
# syft's default is to call Anchore on every run to ask whether a newer syft
# exists. Off at the image level, so the agent's only egress is the StackRadar
# API and the registries it is asked to pull from, whatever the pod spec says.
ENV SYFT_CHECK_FOR_APP_UPDATE=false

# The same uid the chart runs the pod as, so `docker run` and the chart agree
# on what "non-root" means and a file the image cannot read is found here
# rather than in a cluster. Node is PID 1 and handles SIGTERM itself (see
# `installShutdownHandlers` in watch.ts) — no init shim is needed.
USER 65534:65534
