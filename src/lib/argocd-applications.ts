...
export function toInventoryApplication(app: ArgoApplication): InventoryApplication | null {
    ...
    return {
        ...
        namespace,
        tool: "argocd",
        /* Redacted, not raw: a `repoURL` is allowed to carry `user:token@`
           — people do put a PAT in an Application spec — and this row lands
           in the control plane's database and UI. The path and revision are
           what a remediation needs; the credential is not. */
        repoUrl: redactUrlCredentials(str(source?.repoURL)),
...
/** `redactProxyCredentials`, lifted to any URL; `null` stays `null`. */
function redactUrlCredentials(url: string | null): string | null {
    return url === null ? null : redactProxyCredentials(url);
}
