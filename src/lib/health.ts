...
/** What the control plane last said beyond `ok` — surfaced on `/healthz`. */
let heartbeatNotice: { minScannerVersion?: string; message?: string } = {};
export function recordHeartbeatNotice(notice: { minScannerVersion?: string; message?: string }): void {
    heartbeatNotice = {
        ...(notice.minScannerVersion ? { minScannerVersion: notice.minScannerVersion } : {}),
        ...(notice.message ? { message: notice.message } : {}),
    };
}
export function heartbeatNoticeSnapshot(): Readonly<typeof heartbeatNotice> {
    return heartbeatNotice;
}
