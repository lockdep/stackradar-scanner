...
describe("repoURL credentials", () => {
    it("are redacted before the row is built", () => {
        const row = toInventoryApplication({
            metadata: { name: "api", namespace: "argocd" },
            spec: { source: { repoURL: "https://deploy:ghp_secret@github.com/acme/infra", path: "apps/api" } },
        });
        expect(row?.repoUrl).toBe("https://***@github.com/acme/infra");
    });
});
