 ...
 */
/**
 * The CycloneDX spec version the agent emits. Pinned explicitly rather than
 * left to syft's default: the control plane validates uploads against exactly
 * the versions it vendors a schema for, so a syft bump whose default moved to
 * a newer spec would turn every upload from every cluster into a 400 with no
 * change on either side that anyone would think to look at. Bumping this is a
 * contract change — the server must accept the new version first.
 */
export const CYCLONEDX_SPEC_VERSION = "1.6";

/**
 * Environment for the syft child. Two things are forced off regardless of what
 * the pod was given:
 *
 * - `SYFT_CHECK_FOR_APP_UPDATE`: syft's default is to call Anchore on every
 *   run to ask whether a newer syft exists. A supply-chain agent phoning a
 *   third party is exactly what a reviewer with flow logs goes looking for,
