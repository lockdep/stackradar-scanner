...
generated by BuildKit. These live in the image index rather than as cosign
`.att` artifacts, so they are read with `imagetools`:

```bash
docker buildx imagetools inspect \
  ghcr.io/lockdep/stackradar-scanner:1.2.3 --format '{{json .Provenance}}'
docker buildx imagetools inspect \
  ghcr.io/lockdep/stackradar-scanner:1.2.3 --format '{{json .SBOM}}'
```

The release workflow runs the two `cosign verify` commands itself, checks the
attestations and both platforms (`linux/amd64`, `linux/arm64`) are present,
runs the image under the chart's security context, and installs the published
chart into a kind cluster before finishing — so a release that completes has
demonstrably verifiable signatures and has actually run once.
...
The README's install snippet is pinned the same way: the `readme` job rewrites
`--version` in its `helm install` block and pushes that commit straight to
`main` with `GITHUB_TOKEN` (so it raises no push event and runs no CI).

Prereleases are never advertised. A rollback is the manual edit the comments
in those files describe, not a re-run of these jobs.

The gitops job needs `GITOPS_DEVELOPMENT_TOKEN` and `GITOPS_PROD_TOKEN` — the
org-level fine-grained PATs `lockdep/stackradar` uses for image pins.
