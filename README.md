...
pinning one pins exact bytes. The version above is the latest stable release
(the release workflow rewrites it); every version is on the
...
Sent to your StackRadar API endpoint — the SBOM and image identity once per
image on upload, everything else in the inventory report described below:
...
**workload inventory** — the deployment side of the model, and the only place
the labels above actually travel (the SBOM upload carries none). For every
container it would scan:

- namespace, workload name and kind, container name, image reference and digest,
  whether it is an init container;
- the allowlisted labels and annotations listed above, from the pod and from
  its controller;
- the number of running pods behind the workload and when the oldest started;
- the Helm release that packaged it — release name and namespace, chart name
  and version, app version — where the labels name one;
- the ArgoCD application that delivers it — application name and namespace,
  repository URL (with any `user:token@` userinfo redacted), chart, path,
  target revision and destination namespace — where one exists.

It is what lets StackRadar show you "47 workloads discovered, 6 scanned" while
the remaining images are still being pulled, and it is filtered by exactly the
same namespace and image rules as scanning, so anything you exclude never
appears in it either. The full table is on the
[architecture page](https://stackradar.io/docs/architecture#data-leaving).

One thing the SBOM itself carries that is easy to miss: syft records the
image's own `LABEL`s as `syft:image:labels:*` component properties. Most are
OCI metadata — `org.opencontainers.image.source`, `.version`, `.revision` —
and that is what makes base-image recommendations possible, but a label holding
an internal URL or a build identifier travels with the SBOM. Strip them at
build time if that matters to you.
...
no privilege escalation, `seccompProfile: RuntimeDefault` (admissible under the
`restricted` Pod Security Standard), `/tmp` on a size-limited `emptyDir`. The
pod needs egress to your StackRadar endpoint, to the Kubernetes API, and to
whichever registries host your images — and to nothing else. In particular,
syft's default check-in with Anchore for newer versions
(`SYFT_CHECK_FOR_APP_UPDATE`) is switched off in both the image and the chart,
so there is no third-party destination in the agent's flow logs.
...
The image is built for `linux/amd64` and `linux/arm64`. It carries SLSA
provenance and an SBOM as BuildKit attestations in the image index — readable
with `imagetools`, not with `cosign verify-attestation`, which looks for a
separate `.att` artifact the release does not produce:
...
The release workflow checks that both attestations and both platforms are
present before the chart is published.
