...
| watcher.resources | object | `{"limits":{"memory":"1Gi"},"requests":{"cpu":"50m","memory":"512Mi"}}` | Resource requests and limits for the watcher Deployment. No CPU limit is set by default — CPU is compressible, and a limit only throttles syft mid-scan on a node that has cycles to spare. Clusters that enforce LimitRanges or quotas can set one with `watcher.resources.limits.cpu`. |
